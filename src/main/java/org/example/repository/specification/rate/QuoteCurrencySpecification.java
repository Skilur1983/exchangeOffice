...
@Slf4j
@Component
public class QuoteCurrencySpecification implements SpecificationProvider<DayRate> {

    private static final String FILTER_KEY = "quoteCurrency";
    private static final String FIELD_NAME = "quoteCurrency";
    ...
    public Specification<DayRate> getSpecification(String[] params) {
        ...
        try {
            ...
            log.debug("Creating specification for quoteCurrency: {}", currency);

            return (root, query, criteriaBuilder) ->
                    criteriaBuilder.equal(root.get(FIELD_NAME), currency);

        } catch (IllegalArgumentException e) {
            log.warn("Invalid quoteCurrency value: '{}'. Available currencies: {}",
